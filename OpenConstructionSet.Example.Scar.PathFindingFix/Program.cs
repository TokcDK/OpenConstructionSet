...
foreach (var race in races)
{
    ...
    foreach (var ReferenceCategoryInfo in itemsBySoundsID)
    {
        if (!race.ReferenceCategories.ContainsKey(ReferenceCategoryInfo.Key)) race.ReferenceCategories.Add(ReferenceCategoryInfo.Key);

        var refList = race.ReferenceCategories[ReferenceCategoryInfo.Key].References;
        foreach (var reference in ReferenceCategoryInfo.Value)
        {
            if (!refList.ContainsKey(reference.TargetId)) refList.Add(reference.TargetId, GetVal0(race, ReferenceCategoryInfo.Key), GetVal1(race, ReferenceCategoryInfo.Key), GetVal2(race, ReferenceCategoryInfo.Key));
        }
    }
}
